...
# from modules.custom_classes import Race, Archetype, Items
# from modules.functions import pull_saved_data_indexes, pull_saved_data
from modules.custom_classes import Inventory

not_pickle_list = [str, int, float, bool]


def db_create_connection(db_file: str = 'db/dnd.db') -> sqlite3.Connection:
    """
...
def db_select_value(conn, table, value, where, where_what) -> sqlite3.Row:
    """
...
def db_select_values_where(conn, table, value, where, where_what) -> List[sqlite3.Row]:
    """
...
def db_select_values(conn, table, value) -> List[sqlite3.Row]:
    """
    Select multiple rows/values from a table with WHERE clause
...
def db_update_class_in_table(conn, dataclass, table, where, where_what):
    """
...
def db_return_class_object(conn, table, where_field, where_what, dataclass):
    """
...
def db_delete_row(conn, table, where, where_what):
    """
...
def db_update_value(conn, table, value, update_value, where, where_what):
    """
